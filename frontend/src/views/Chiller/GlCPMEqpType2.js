...
import Cooling_tower_img from "./../../assets/img/CoolingTowerImg.png"
// import Cooling_tower_img from "./../../assets/img/Shiva- Cooling Tower 2.jpg";
// import Cooling_tower_img from "./../../assets/img/Shiva_Cooling tower.jpg";
...
export default function GlChillerPage(props) {
  ...
  const normalizeStatus = (val) => {
    ...
    if (s === "active" || s === "on" || s === "1" || s === "true")
      return "active";
    if (s === "inactive" || s === "off" || s === "0" || s === "false")
      return "inactive";
  ...
  const findAttributeKey = (attrs, param) => {
    if (!attrs) return undefined;
    const lower = String(param).toLowerCase();
    let key = Object.keys(attrs).find((k) => k.toLowerCase() === lower);
    if (key) return key;
  ...
  const [coolingTowerState, setCoolingTowerState] = useState(null);
  ...
  useEffect(() => {
    if (selectedChId) {
      ...
      fetchCoolingTowerState(selectedChId);
      let req = {
        startdate: "start",
        enddate: "end",
      };
      api.floor
        ...
        .then((response) => {
          console.log("[GlCPMEqpType2] Graph data fetched successfully");
        ...
        .catch((error) => {
          console.log("[GlCPMEqpType2] Error fetching graph data:", error);
  ...
  const handleChillerChange = (name, id) => {
    ...
    Object.values(allEquipmentData).map((res) => {
      if (id === res.id) {
        ...
        const attrs_local = res.Eqp_Attributes || {};
        const statusKey_local = findAttributeKey(attrs_local, "CT_Fan_Run_SS");
        const chillerStatus = statusKey_local
          ? attrs_local[statusKey_local]?.presentValue
          : undefined;
        const isChillerActive = chillerStatus === "active";
        setcheckEqpActiveParam(isChillerActive);
        imageParams.forEach((respp) => {
          const attrs = res.Eqp_Attributes || {};
          const attrKey = findAttributeKey(attrs, respp.parameter);
          const attribute = attrKey ? attrs[attrKey] : undefined;
          const paramName = respp.parameter || "";
          const isSpeedParam = /actual_speed|speed|var_fan/i.test(paramName);
          const forceShow = !!respp.showWhenPresent || isSpeedParam;

          if (attribute !== undefined) {
            const pv = attribute.presentValue;
            let value = "-";
            if (forceShow || isChillerActive) {
              if (pv !== undefined && pv !== null && pv !== "") {
                const num = Number(pv);
                value = !isNaN(num) ? formatter1.format(num) : String(pv);
              } else {
                value = "-";
              }
            } else {
            ...
            eqpParams1.push({
              ...respp,
              value,
              unit: forceShow ? respp.unit : isChillerActive ? respp.unit : "",
            });
          } else {
        ...
        // })
        let eqpParams2 = [];
        const metrics = res.Eqp_Metrics || {};
        const attrs = res.Eqp_Attributes || {};
 
        controlsCard.forEach((respp) => {
          let value = respp.defaultValue;
 
          // Special handling for Level Status
          if (respp.title === "Level Status" && respp.parameterLow) {
            const highAttrKey = findAttributeKey(attrs, respp.parameter);
            const lowAttrKey = findAttributeKey(attrs, respp.parameterLow);
           
            const high = highAttrKey ? attrs[highAttrKey]?.presentValue : undefined;
            const low = lowAttrKey ? attrs[lowAttrKey]?.presentValue : undefined;
 
            // Determine display value based on high and low status (same logic as summary page)
            ...
            if (metrics[respp.parameter] !== undefined) {
              value = metrics[respp.parameter];
            } else {
              const attrKey = findAttributeKey(attrs, respp.parameter);
              const attr = attrKey ? attrs[attrKey] : undefined;
 
              if (attr && attr.presentValue !== undefined) {
                const raw = attr.presentValue;
                if (respp.section === "Trip Status") {
                  value = raw;
                } else {
                  value = normalizeStatus(raw);
                }
      ...
      .then((res) => {
        ...
        controlsCard.forEach((respp) => {
          ...
          eqpParams2.push({
            ...
            console.log("atttttttttttttttt", eqpParams5);
            setParticularEquipDataSet5(eqpParams5);
            let eqpParams6 = [];
            controlsCardLeft2.map((respp) => {
              const attribute = res.Eqp_Attributes[respp.parameter];
    ...
    const timer = setInterval(() => {
      ...
      if (selectedChIdRef.current) {
        ...
        api.floor
          ...
          .then((resp) => {
           if (resp[eqpType]) {
          let data = Object.values(resp[eqpType]).sort(function (a, b) {
            return a.name > b.name ? 1 : b.name > a.name ? -1 : 0;
          });
          setAllEquipmentData(data);
        }
  ...
  const fetchCoolingTowerState = async (ss_id) => {
    if (!ss_id) return Promise.resolve(null);
    ...
    try {
      ...
      const s =
        response !== null && response !== undefined
          ...
          : null;
      const isOn =
        s === "active" || s === "on" || s === "1" || s === "1.0" || s === true;
      setOnOff2(isOn ? 1 : 0);
      // setOnOff2(response);
    } catch (e) {
  ...
  const onChange = (newValue) => {
    ...
    const req = {
      ...
      ss_id: id,
      param_id:
        props.eqpType === "NONGL_SS_COOLING_TOWER"
          ? "cmd_vlv_on_off_00"
          : "",
      gl_command,
      value: msg,
    ...
    const req1 = {
      ...
      ss_id: id,
      param_id:
        props.eqpType === "NONGL_SS_COOLING_TOWER" ? "cmd_on_off_00" : "",
      gl_command,
      value: msg,
      zone_type: null,
      zone_id: null,
  ...
  const ChipMethod = (props) => {
    ...
    // )
    {
      ...
      backgroundColor = isTripped ? "#f44336" : "#9e9e9e";
    }
    ...
    return (
      ...
      >
        ...
        >
          ...
          ) : props.title === "Inlet Valve-1 Status"||
          props.title === "Inlet Valve-2 Status" ? (
            <>
          ...
          ) : props.title === "Outlet Valve Status" ? (
            <>
              {props.value === "inactive" ||
              props.value === "closed" ||
              parseInt(props.value) === 0
            ...
            props.title == "Fan Mode" ||
            props.title == "Fan 1 Mode"||
            props.title == "Fan 2 Mode" ? (
            <>{props.value === null || props.value === undefined
                ? "N/A"
                :props.value === "active" || parseInt(props.value) === 1 || props.value === "1"
                ? "Auto"
            ...
            <>{props.value === "inactive" ? "OFF" : "ON"}</>
          ) : props.title === "Run Status" || props.title === "Fan Status" ? (
            <>
              {props.value === null || props.value === undefined
              ? "N/A"
              :props.value === "active" || parseInt(props.value) === 1 || props.value === "1"
  ...
  const handleClickRat = (event) => {
    ...
    const req = {
      ...
      param_id: props.eqpType == "NONGL_SS_CHILLER" ? "Set_EWT" : "",
      value: setPointvalue,
      zone_type: null,
      zone_id: null,
    };
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                      ...
                      >
                        ...
                        {particularEquipDataSet5 != undefined ? (
                          <>
                            <Marker
                              position={window.innerWidth =='1920'?[256.9, 275.8]:[254, 306]}
...
>
  <div style={{
    ...
    {particularEquipDataSet5.map((res, index) => (
      <div
        key={res.title}
        style={{
          display: "flex",
      ...
      >
        ...
        <span style={{
          ...
          {res.unit ? ` ${res.unit}` : ""}
                        ...
                        {console.log("allEquipmentData", allEquipmentData)}
                        {Object.values(allEquipmentData).map((element) =>
                          selectedChId === element.id ? (
                            <React.Fragment key={`valves-${element.id}`}>
                              {/* Inlet Valve */}
                              <Marker
                                position={window.innerWidth =='1920'?[160.8, 348.8]:[265, 223]}
                                // position={[283, 287]}
                                icon={
                                  element?.Eqp_Attributes?.sts_vlv_on_off_02
                                    ?.presentValue === "active" ||
                                  element?.Eqp_Attributes?.sts_vlv_on_off_02
                                    ?.presentValue === "1" ||
                                  element?.Eqp_Attributes?.sts_vlv_on_off_02
                                    ?.presentValue === 1
                                    ? iconDevice2 // green blinking
                                    : iconDevice3 // red
                                }
                              >
                              ...
                              {/* Outlet Valve */}
                              <Marker
                                position={window.innerWidth =='1920'?[258.9, 154.9]:[209, 351]}
                                // position={[218, 445]}
                                icon={
                                  element?.Eqp_Attributes?.sts_vlv_on_off_01
                                    ?.presentValue === "active" ||
                                  element?.Eqp_Attributes?.sts_vlv_on_off_01
                                    ?.presentValue === "1" ||
                                  element?.Eqp_Attributes?.sts_vlv_on_off_01
                              ...
                              >
                                <Tooltip direction="right" opacity={0.75}>
                                  <div>
                                    ...
                                    >
                                      Inlet Valve-1 Status:{" "}
                                      {element?.Eqp_Attributes
                                        ?.sts_vlv_on_off_01?.presentValue ===
                                        "active" ||
                                      element?.Eqp_Attributes?.sts_vlv_on_off_01
                                        ?.presentValue === "1" ||
                                      element?.Eqp_Attributes?.sts_vlv_on_off_01
          ...
          <Grid item xs={12} sm={12} md={3} lg={3} xl={3} xxl={3}>
            <Grid container item xs={12} spacing={1} direction="column">
              <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                ...
                </Paper>{console.log("particularEquipDataSet2",particularEquipDataSet2)}
              </Grid>
              <Grid item xs={12} sm={12} md={12} lg={12} xl={12} xxl={12}>
                <Card
                  className={classes.paper}
                                ...
                                <Grid item xs={4} sm={4} md={4} lg={4} xl={4}>
                                  {res.component == "Set Operator" ? (
                                    ...
                                    <>
                                      ...
                                      ) : res.title == "Cooling Tower" ||
                                        res.title == "Cooling Tower Fan" ||
                                        res.title == "Cooling Tower Fans" ? (
                                     ...
                                     className={classes.switchselector}> */}
                                        ...
                                        "Cooling Tower Outlet Valve Position" ? (
                                        <div
                                          className={classes.switchselector}
                                          style={{
                                            pointerEvents:
                                    ...
                                    >
                                      ...
                                      >
                                        <TextField
                                          // label="%"
                                          placeholder={
                                            formatter.format(res.value) >= 5 &&
                                            formatter.format(res.value) <= 20
                                              ? formatter.format(res.value)
                                              : res.defaultValue + "℃"
                                          }
                                          // style={{width:'8.5vh'}}
                  ...
                  {false &&
                    (function () {
                      ...
                      const fans = [
                        {
                          id: "Fan-1",
                          runParam: "CT_Var_FAN_1_On_Off_SS",
                          tripParam: "CT_Var_FAN_1_Trip_SS_Alarm",
                        },
                        {
                          id: "Fan-2",
                          runParam: "CT_Var_FAN_2_On_Off_SS",
                          tripParam: "CT_Var_FAN_2_Trip_SS_Alarm",
                        },
                        {
                          id: "Fan-3",
                          runParam: "CT_Var_FAN_3_On_Off_SS",
                          tripParam: "CT_Var_FAN_3_Trip_SS_Alarm",
                        },
                      ];
                      ...
                      const fanData = particularEquipDataSet2
                        .filter(
                          (item) =>
                            item.parameter &&
                            (item.parameter.includes("CT_Fan") ||
                              item.parameter.includes("CT_Var_FAN"))
                        )
                        .map((item) => ({
                      ...
                      const trip1 = particularEquipDataSet2.find(
                        (r) => r.parameter === "CT_Var_FAN_1_Trip_SS_Alarm"
                      );
                      const trip2 = particularEquipDataSet2.find(
                        (r) => r.parameter === "CT_Var_FAN_2_Trip_SS_Alarm"
                      );
                      const trip3 = particularEquipDataSet2.find(
                        (r) => r.parameter === "CT_Var_FAN_3_Trip_SS_Alarm"
                      );
                      console.log("[FanStatusDebug] Trip params found:", {
                        "Fan-1": trip1 ? trip1.value : "NOT FOUND",
                        "Fan-2": trip2 ? trip2.value : "NOT FOUND",
                        "Fan-3": trip3 ? trip3.value : "NOT FOUND",
                      });
                      console.log("[FanStatusDebug] Trip1 full:", trip1);
                      console.log("[FanStatusDebug] Trip2 full:", trip2);
                      console.log("[FanStatusDebug] Trip3 full:", trip3);

                      if (
                        !particularEquipDataSet2 ||
                        particularEquipDataSet2.length === 0
                      )
                        return null;
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {fans.map((f) => {
                              const isSelected = selectedFan === f.id;
                              const runItem = findByParam(f.runParam);
                              const tripItem = findByParam(f.tripParam);

                              // Debug: log each fan's data and verify it found the right parameter
                              const tripParamMatch =
                                tripItem?.parameter === f.tripParam
                                  ? "✓"
                                  : "✗ WRONG";
                              console.log(
                                `[Fan Color] ${f.id}: trip param=${
                                  tripItem?.parameter
                                } (expected ${
                                  f.tripParam
                                  ...
                                  tripItem &&
                                  tripItem.value !== undefined &&
                                  tripItem.value !== "inactive"
                                    ? "RED"
                                    : runItem && runItem.value === "active"
                                    ? "GREEN"
                                    : "GREY"
                                }`
                              );
                              ...
                              let cardColor = "#9e9e9e"; // grey (default/off)
                              ...
                              if (
                                ...
                                cardColor = "#f44336"; // red for tripped
                              }
                              // Check if on (green)
                              else if (runItem && runItem.value === "active") {
                                cardColor = "#4caf50"; // green for on
                              }
                              ...
                              return (
                                ...
                                >
                                  <ButtonBase
                                    ...
                                    onClick={() =>
                                      setSelectedFan(isSelected ? null : f.id)
                                    }
                                  >
                                    <Paper
                                  ...
                                  >
                                    ...
                                    {(() => {
                                      ...
                                      return (
                                        <ChipMethod
                                          {...runItem}
                                          title={mapping.id} // Use Fan-1, Fan-2, Fan-3 as title
                                          section={"Fan Status"}
                                          textSize={"1.4vh"}
                                        />
                                      );
