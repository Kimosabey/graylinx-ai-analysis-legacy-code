...
function addParameterBoxes() {
    return <>
        ...
        <rect x={290} y={12} width={50} height={55} fill='#00f'></rect>
        ...
        <rect x={88} y={105} width={35} height={20} fill='#00f'></rect>
...
function addBoxedParameters(textboxes = [
    { 'x': 440, 'y': 80, 'width': 150, 'height': 55, 'color': '#00f', 'textColor': '#fff', 'textSize': "0.75em", 'textitems': ["S/Air Temperature S/P: 14.4 C", "S/Air Pressure S/P: 355 Pa", "Mix Pressure S/P: -175 Pa"] },
    ...
    return textboxes.map(box => {
        myx = box['x']; myy = box['y'];
        return <>
            <rect x={myx} y={myy} width={box['width']} height={box['height']} fill={box['color']}></rect>
            {box['textitems'].map(item => {
                myy += textYd;
                return <text x={myx + textXd} y={myy} fontSize={box['textSize']} stroke={box['textColor']}>{item}</text>
            }
            )}
        </>
    })
...
function prepareView(myImages = [], w = 120, h = 100, startx = 0, starty = 0, tw = 900, th = 200, arrow = 8, addBoxes = false) {
...
export default function GRHA({
    ...
    return (
        <><div className="App">
            ...
            ], 0, 0, 0, 0, width + boxWidth, height + boxHeight, 0, addBoxes)}
        </div></>
    );
}
