...
const createarea = (payload, callback) => {
    pool.getConnection((err, connection) => {
      if (connection) {
        const query = 'SELECT * FROM area WHERE name = ? AND zone_id = ?';
        connection.query(query, [payload.name, payload.zone_id], (error, results) => {
          if(results.length > 0)
          {
            connection.release();
          ...
          {
            connection.query('INSERT INTO area SET ?', payload, (error1, results1) => {
              connection.release();
              if (error1) {
                console.log("messsasge",error1.message)
                callback(error1.message);
              } else {
                logger.info("ACTION: Create Zone; ZONE ID: "+payload.id+"; RESULT: Success; USER: admin; ROLE: super_admin")
  ...
  const editAreaName = (area, callback) => {
    pool.getConnection((err, connection) => {
      if(connection) {
        connection.query(`select count(*) as count from area where name = ? and zone_id = ?`, [area.name, area.parent_id], (error, countsResult) => {
          if(countsResult[0].count > 0) {
            callback({ message: `'${area.name}' already exists`, status: 422 });
          }
          else {
            connection.query(
              'UPDATE area SET name = ? where id = ?',
              [area.name, area.id],
              (update_area_error, results) => {
                connection.release();
                if (update_area_error) {
                  callback(update_area_error);
                } else {
                  if (results.affectedRows === 1) {
                    logger.info("ACTION: Edit Area Name; AREA ID: "+area.id+
                      "; RESULT: Success; USER: admin; ROLE: super_admin")
                    callback(null, results);
                  } else {
                    logger.error("ACTION: Edit Area Name; AREA ID: "+area.id+
                      "; RESULT: Area Not Found; USER: admin; ROLE: super_admin")
                    callback({ message: 'Area Not Found', status: 404 });
                  }
                }
              }
            );
        ...
        connection.query(
          ...
          (error, results) => {
            ...
            if (error) {
              ...
              if (results.affectedRows === 1) {
                logger.info("ACTION: Delete Area; AREA ID: "+areaId+
                  "; RESULT: Success; USER: admin; ROLE: super_admin")
                callback(null, results);
              } else {
                logger.error("ACTION: Delete Area; AREA ID: "+areaId+
                  "; RESULT: Area Not Found; USER: admin; ROLE: super_admin")
                callback({ message: 'Area Not Found', status: 404 });
              }
            }
  ...
  const lights = (buildingId, callback) => {
    pool.getConnection((error, connection) => {
      if (error) {
        ...
        const query =
        "select d.mac,d.name,d.type,d.device_info,d.area_id,a.name as areaName,z.id as zoneId ,z.name as zone_name,l.data,l.created_at from device d left join latest_event l on l.device_id=d.id inner join area a on a.id=d.area_id inner join zone z on  a.zone_id=z.id    where a.id=? and ( d.type='dali_slave' or d.type='analog_controller') union all select d.mac,d.name,d.type,d.device_info,d.area_id,a.name as areaName,z.id as zoneId,z.name as zone_name,l.data as data,l.created_at as created_at from device d left join latest_event l on l.device_id=d.id  inner join area a on d.area_id=a.id inner join zone z on a.zone_id=z.id   where  d.type='dali_master'"
