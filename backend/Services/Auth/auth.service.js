...
const login = (user, callback) => {
  ...
  if(user.credentials.role) {
    console.log("selef mode")
    model.selfModeLogin(user, (error, result) => {
      if (error) {
        callback(error);
      }
...
const registerUser = (user, callback) => {
  hash(user.password, (error, passwordHash) => {
    if (error) {
      ...
      const role_id = Object.freeze({ superAdmin: 1, admin: 2, user: 3, cluster_manager: 4, hq_manager:5});
      const payload = {
        id: uuid(),
      ...
      model.createUser(payload, _error => {
        if (_error) {
          if(_error.code === 'ER_DUP_ENTRY')
            ...
            callback('User Registration failed / Invalid Campus ID');
        } else {
          callback(null);
        }
      });
...
const addLicenceDates = (data,callback)=>{
  ...
  model.addLicenceDates(data,(err,results)=>{
      if(err){
          callback(err)
      }else{
