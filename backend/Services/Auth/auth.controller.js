...
const login=(req, res, next)=> {
  ...
  if(!errors.isEmpty()) {
    ...
    const user= req.body;
      if(lockout.hasOwnProperty(`${user.credentials.username}`) && lockout[`${user.credentials.username}`].hasOwnProperty('isLocked') && lockout[`${user.credentials.username}`].isLocked && lockout[`${user.credentials.username}`].hasOwnProperty('expiresIn') && moment(lockout[`${user.credentials.username}`].expiresIn).isSameOrAfter(moment().format('YYYY-MM-DDTHH:mm:ss'))) 
      {
        res
        .status(400)
        ...
        service.login(user, (error, response) => {
          if (error) {
            if (error ==='Invalid credentials') {
              if (lockout.hasOwnProperty(`${user.credentials.username}`) && lockout[`${user.credentials.username}`].hasOwnProperty('count') && lockout[`${user.credentials.username}`].hasOwnProperty('isLocked')) 
              {
                lockout[`${user.credentials.username}`] = {};
              } else if (lockout.hasOwnProperty(`${user.credentials.username}`) && lockout[`${user.credentials.username}`].hasOwnProperty('count')) {
                ...
                if (lockout[`${user.credentials.username}`].count===5) {
                  ...
                  lockout[`${user.credentials.username}`].expiresIn =format(addMinutes( new Date(),15),'yyyy-MM-dd HH:mm:ss'
                  );
                }
              } else {
                lockout[`${user.credentials.username}`] = {
...
const superAdmin=(req, res, next)=> {
  ...
  if(!errors.isEmpty()) {
    ...
    const user= req.body;
    console.log("=====cred",`${user.credentials.password}`)
    if(lockout.hasOwnProperty(`${user.credentials.username}`) && lockout[`${user.credentials.username}`].hasOwnProperty('isLocked') && lockout[`${user.credentials.username}`].isLocked && lockout[`${user.credentials.username}`].hasOwnProperty('expiresIn') && moment(lockout[`${user.credentials.username}`].expiresIn).isSameOrAfter(format(new Date(), 'yyyy-MM-dd HH:mm:ss'))) 
    {
        res
        .status(400)
        .json({ errors: { global:'Account Locked for 15 mins' } });
        ...
        service.superAdmin(user, (error, response) => {
          if (error) {
            if (error ==='Invalid credentials') {
              ...
              if (lockout[`${user.credentials.username}`].count===5) {
                lockout[`${user.credentials.username}`].isLocked=true;
                lockout[`${user.credentials.username}`].expiresIn=moment().add(15, 'm').format('YYYY-MM-DDTHH:mm:ss');
              }
            } else {
              lockout[`${user.credentials.username}`] = {
                count:1
...
const addLicenceDates = (req, res, next) => {
  const data = req.body;
  service.addLicenceDates(data,(error, result) => {
    if (error) {
      next(error);
    } else {
