...
connection.query(
  `CREATE TABLE IF NOT EXISTS device (
        ...
        KEY fk_device_zone_idx (zone_id),
        KEY device_type_zone_id_idx (type,zone_id),
        CONSTRAINT fk_device_zone FOREIGN KEY (zone_id) REFERENCES zone (id) ON DELETE CASCADE
      )`,
  error => {
    if (error) {
      logger.error(error.message);
