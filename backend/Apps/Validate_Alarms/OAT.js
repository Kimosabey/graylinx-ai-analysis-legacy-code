      ...
      (response, callback) => {
        ...
        if (response.length > 0) {
          response.forEach(devId => {
            if (
              devId.message == 'Outside Air Temperature Mismatch'
              ) {
          ...
          if(err){
            ...
            let obj = {};
              lateres.forEach(current=>{
                if(response[0] == 'OAT is Low'){
                    if(current.param_id == 'OAT'){
                        if(current.param_value < 10){
                            obj['OAT sensor'] = 'Faulty'
                        }else{
                            obj['OAT sensor'] = 'Fine'
                            console.log("OAT sensor working Fine")
                        }
                    }
              }
              })
