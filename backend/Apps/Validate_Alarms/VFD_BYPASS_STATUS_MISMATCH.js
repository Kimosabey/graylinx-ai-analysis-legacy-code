      ...
      (response, callback) => {
        ...
        if (response.length > 0) {
          response.forEach(devId => {
            if (
              devId.message == 'VFD Bypass Status - (Fan motor through VFD ? Direct "bypass" ?) Mismatch'
              ) {
                                  ...
                                  (err, results2) => {
                                    if (err) {
                                      ...
                                      // console.log("results2",results2)
                                        ...
                                        if(comp.param_id == 'VFD_Byp_SS'){
                                          console.log('valueeeVFD_Byp_SS',comp.param_value);
                                          if(comp.param_value == 1){
                                            arr.push('VFD_Byp_SS Mismatch');
                                            arr.push(devId.id);
                                            ...
                                            console.log('VFD_Byp_SS Mismatch');
                                          }else{
                                            arr.push('VFD_Byp_SS is good')
                                            console.log("VFD_Byp_SS is good")
                                          }
                                        }
                                    }
                                  }
          ...
          if(err){
            ...
            inres.forEach(input=>{
              lateres.forEach(current=>{
                if(response[0] == 'VFD_Byp_SS Mismatch'){
                    ...
                    if(input.param_id == 'SAF_VFD_Speed' && current.param_id == 'SAF_VFD_Speed_Fbk'){
                        if(input.param_value - current.param_value > 10){
                          obj['Possible cause 5'] = 'AHU Fan Speed Mismatch'
