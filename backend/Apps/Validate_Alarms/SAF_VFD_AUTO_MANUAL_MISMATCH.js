      ...
      (response, callback) => {
        ...
        if (response.length > 0) {
          response.forEach(devId => {
            if (
              devId.message == 'Supply Air Fan VFD-Auto/Manual-Command Mismatch'
              ) {
                                  ...
                                  (err, results2) => {
                                    if (err) {
                                      ...
                                      results2.forEach(input => {
                                        ...
                                        if(comp.param_id == 'SAF_VFD_AM_Fbk' && input.param_id =='SAF_VFD_AM'){
                                          console.log('valueeeSAF_VFD_AM',comp.param_value);
                                          if(comp.param_value == input.param_value){
                                            arr.push('SAF_VFD_AM is Good');
                                            // callback(null, arr);
                                            console.log('SAF_VFD_AM IS GOOD');
                                            // console.log("alarm_code",devId.alarm_code)
                                            }else{
                                            arr.push('SAF_VFD_AM Mismatch');
                                            arr.push(devId.id);
                                            ...
                                            console.log('SAF_VFD_AM Mismatch');
                                          }
                                        }
                                      });
                                    }
          ...
          if(err){
            ...
            inres.forEach(input=>{
              lateres.forEach(current=>{
                if(response[0] == 'SAF_VFD_AM Mismatch'){
            ...
            causes['SAF_VFD_AM Mismatch'] = obj;
