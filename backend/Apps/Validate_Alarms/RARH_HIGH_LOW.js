      ...
      (response, callback) => {
        ...
        if (response.length > 0) {
          response.forEach(devId => {
            if (
              devId.message == 'Return Air Relative Humidity Mismatch'
              ) {
                                  ...
                                  (err, results2) => {
                                    if (err) {
                                      ...
                                      // console.log("results2",results2)
                                        ...
                                        if (comp.param_id == 'RARH') {
                                          console.log('valueeeRARH',comp.param_value);
                                          if (comp.param_value > 60) {
                                            arr.push('RARH is High');
                                            arr.push(devId.id);
                                            ...
                                            console.log('RARH IS Low');
                                          }else{
                                            arr.push('RARH is Good');
                                            // callback(null, arr);
                                            console.log('RARH IS GOOD');
                                          }
                                        }
                                    }
                                  }
          ...
          if(err){
            ...
            inres.forEach(input=>{
              lateres.forEach(current=>{
                if(response[0] == 'RARH is High' || response[0] == 'RARH is Low'){
                ...
                if(current.param_id == 'RARH'){
                  if(current.param_value < 30){
                    obj['RARH sensor'] = 'Faulty'
                  }else if(current.param_value > 60){
                    obj['RARH sensor'] = 'Faulty'
                  }else{
                    obj['RARH sensor'] = 'Fine'
