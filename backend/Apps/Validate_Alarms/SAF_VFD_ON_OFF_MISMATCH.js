      ...
      (response, callback) => {
        ...
        if (response.length > 0) {
          response.forEach(devId => {
            if (
              devId.message == 'Supply Air Fan VFD - On / Off - Command Mismatch'
              ) {
                                  ...
                                  (err, results2) => {
                                    if (err) {
                                      ...
                                      results2.forEach(input => {
                                        ...
                                        if(comp.param_id == 'SAF_VFD_On_Off_Fbk' && input.param_id == 'SAF_VFD_On_Off'){
                                          console.log('valueeeSAF_VFD_On_Off',comp.param_value);
                                          if(comp.param_value == input.param_value){
                                            arr.push('SAF_VFD_On_Off is Good');
                                            console.log('SAF_VFD_On_Off IS GOOD');
                                            // console.log("alarm_code",devId.alarm_code)
                                          }else{
                                            arr.push('SAF_VFD_On_Off Mismatch');
                                            arr.push(devId.id);
                                            ...
                                            console.log('SAF_VFD_On_Off Mismatch');
                                          }
                                        }
                                      });
                                    }
          ...
          if(err){
            ...
            let obj = {};
              lateres.forEach(current=>{
                if(response[0] == 'SAF_VFD_On_Off Mismatch'){
                    if(current.param_id == 'SAF_VFD_AM_Fbk'){
                        if(current.param_value == 1){
                            obj['Possible cause 1'] = 'AHU is in Manual mode'
                        }else{
                            console.log("AHU is in Auto mode")
                        }
                    }
              }
              })
            causes['SAF_VFD_On_Off Mismatch'] = obj;
