      ...
      (response, callback) => {
        ...
        if (response.length > 0) {
          response.forEach(devId => {
            if (
              devId.message == 'Return Air Temperature Mismatch'
              ) {
                                  ...
                                  (err, results2) => {
                                    if (err) {
                                      ...
                                      results2.forEach(input => {
                                        ...
                                        if (comp.param_id == 'RAT' &&
                                        input.param_id ==
                                          'RAT_SP') {
                                          console.log('valueeeRAT',comp.param_value);
                                          if (comp.param_value > 27) {
                                            arr.push('Rat is High');
                                            arr.push(devId.id);
                                            ...
                                            console.log('RAT IS Low');
                                          }else{
                                            arr.push('Rat is Good');
                                            // callback(null, arr);
          ...
          if(err){
            ...
            inres.forEach(input=>{
              lateres.forEach(current=>{
                ...
                if(current.param_id == 'RAT'){
                  if(current.param_value < 10){
                    obj['RAT sensor'] = 'Faulty'
                  }else if(current.param_value > 40){
                    obj['RAT sensor'] = 'Faulty'
                  }else{
                    obj['RAT sensor'] = 'OK'
                ...
                if(input.param_id == 'RAT_SP' && current.param_id == 'RAT'){
                  if(input.param_value - current.param_value > 1.5){
                    ...
                    console.log("CWL PID loop convergence is ok")
                  }
                }
                // if(current.param_id == 'SAF_VFD_AM_Fbk'){
                //   if(current.param_value == 1){
                //     obj['Exhaust Damper Position'] = 'Closed'
                //   }else{
                //     obj['Exhaust Damper Position'] = 'Auto'
                //     console.log('Exhaust Damper Position is in Auto')
                //   }
                // }
                ...
                obj['High Thermal Load inside the Building'] = 'Check'

              }else if(response[0] == 'Rat is Low'){
                if(input.param_id == 'CHW_Vlv_Pos_SP' && current.param_id == 'CHW_Vlv_Pos'){
                  if(input.param_value - current.param_value > 10){
                    obj['Chilled Water Control Valve Operation'] = 'Faulty'
                  }else if(input.param_value - current.param_value < -10){
