...
const { loadEqpDetails, getTableName } = require("../Config/common");
let eqpDetails = loadEqpDetails();
console.log(
  "preparePS--Summary of Equipment - ",
  typeof loadEqpDetails,
  typeof eqpDetails
);
...
function createChild(id, res) {
  ...
  childrenId = children.map((mda) => {
    ...
    parentId =
      mda.glSSId.substring(childIndex + 1, childIndex + 3) +
      "b7".padEnd(6, "0");
...
const prepareTableName = (glCode, devType, tableType) => {
  ...
  const deviceTypeMetric = eqpDetails["EQP_METRICS"];
  ...
  let eqpParamDtls = getTableName("GL" + glCode);
  console.log(
    `glCode - ${glCode},devType-${devType},tableType-${tableType}`,
    eqpParamDtls
  );
  let tableName =
    tableType === "metric"
  ...
  tableName = eqpParamDtls["eqp_metricTableName"];
  return tableName;
};
                ...
                alres.forEach((ele, i) => {
                  ...
                  if (i + 1 == alres.length) {
                    ...
                    for (const obj of res) {
                      ...
                      const roundedNumber =
                        Math.round(metric_value * 100) / 100; //by 2 digit after decimal
...
const init = (master, mycallback) => {
  ...
  let etypes = eqpDetails["EQP_TYPES"];
  ...
  executeSQLstatement(query, async (err, ru) => {
    if (err) {
      ...
      const groupedData = await ru.reduce(async (accPromise, item) => {
        ...
        if (
          ssType !== null &&
          ssType !== "GL_SS_ADDRESS_BACNET_DDC" &&
          ssType != "GL_WEATHER_SERVICE" &&
          ssType !== "GL_SS_WPIR_TYPE_01" &&
          ssType !== "GL_SS_THLSENSOR_TYPE_01"
        ) {
          ...
          // const metricData=[]
          let metricData = await getMetricData(
            item.glSSId,
            item.name,
            ssType,
            item.id
