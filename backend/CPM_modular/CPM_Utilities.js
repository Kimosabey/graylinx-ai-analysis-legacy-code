...
let myError = (x, y = "") => {
  console.log("\x1b[33m%s\x1b[0m", `${x} ${myPrint(y)}`);
  myappend(`${x} ${myPrint(y)}`);
};
let myDebug = (x) => myappend(x);
let mySuccess = (x, y = "") =>
  console.log("\x1b[36m%s\x1b[0m", `${x} ${myPrint(y)}`);
let myTempDebug = (x, y = "") =>
  console.log("\x1b[36m%s\x1b[0m", `${x} ${myPrint(y)}`);
let myTable = (x, cols = null) =>
  cols !== null ? console.table(x, cols) : console.table(x);
let sudhu = (x) => console.log(c.bgMagenta(`${x}`));
